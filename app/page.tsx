  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-semibold text-zinc-800">工作台</h2>
        <button
          onClick={handleReset}
          className="px-3 py-1.5 text-xs border border-zinc-300 text-zinc-500 rounded hover:bg-zinc-100 transition-colors"
        >
          重置 Demo 数据
        </button>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        {cards.map((card, i) => (
          ...
          >
            ...
            <div className="text-3xl font-bold text-zinc-800">{card.value}</div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white border border-zinc-200 rounded-lg p-5">
        ...
        <div className="grid grid-cols-4 gap-3">
          <Link
            href="/members"
            className="block p-3 border border-zinc-200 rounded text-center text-sm text-zinc-600 hover:bg-zinc-50 hover:border-blue-300 transition-colors"
          >
            会员管理
          </Link>
          <Link
